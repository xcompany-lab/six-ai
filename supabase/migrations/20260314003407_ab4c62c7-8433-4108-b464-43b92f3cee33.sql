...
BEGIN
  ...
  -- Robust parse: pure numbers treated as hours, 'h' suffix = hours, 'm' suffix = minutes
  ...
  -- If still purely numeric (no unit was matched), treat as hours
  IF raw_first ~ '^\d+$' THEN
...
-- Fix existing bad data: pure numeric values without suffix
UPDATE reminders_config SET first_reminder = first_reminder || 'h' WHERE first_reminder ~ '^\d+$';
UPDATE reminders_config SET second_reminder = second_reminder || 'h' WHERE second_reminder ~ '^\d+$';
